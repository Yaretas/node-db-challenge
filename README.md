...
- [ ] Explain the difference between `Relational Databases` and `SQL`.
      In relational databases, the data is stored in tabular format grouped into rows and columns. A collection of rows is called a table.
      SQL - Atructured Query Language use to acces and control RDSM
      The difference is that SQL is a language used to manipulate the database and RDMS is the software that manages the database server.

- [ ] Why do tables need a `primary key`?

      The primary key is a column in a table which must contain a unique value which can be used to identify each and every row of a table uniquely.
